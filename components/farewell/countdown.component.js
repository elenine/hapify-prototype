...
window.sectionComponents.countdown = {
    ...
    render: (data, style) => {
            ...
            if (diff > 0) {
                countdown = `
                    <div class="bg-white rounded-2xl p-8 shadow-lg inline-block">
                        <div class="text-7xl font-bold text-violet-600 mb-2">${diff}</div>
                        <div class="text-xl text-gray-700 font-semibold">Day${diff !== 1 ? 's' : ''} Until the Party!</div>
                    </div>
                `;
            } else if (diff === 0) {
                countdown = '<div class="text-4xl font-bold text-violet-600 animate-pulse">It\'s Party Time! 🎉🎊</div>';
            } else {
        ...
        return `
            <div class="py-12 px-6 text-center" style="background: ${style.bg || '#f5f3ff'}">
