// Housewarming Components Loader
...
console.log('🔄 Housewarming component loader started');

// List of all available housewarming components
const componentFiles = [
    'layout', 'hero', 'homedetails', 'partyinfo', 'housetour',
    'directions', 'whattobring', 'rsvp', 'homephotos', 'thankyou'
];

// Function to check if all components are loaded
function checkComponentsLoaded() {
    console.log('🔍 Checking if housewarming components are loaded...');
    ...
    console.log(`Housewarming components loaded: ${loaded.length}/${componentFiles.length}`);

    if (missing.length > 0) {
        console.warn('Missing housewarming components:', missing.join(', '));
    } else {
        console.log('✅ All housewarming components loaded successfully!');
    }
