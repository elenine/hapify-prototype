// Greeting Card Components Loader
// This script loads all greeting card section components and makes them available globally

// Initialize the components object if it doesn't exist
window.sectionComponents = window.sectionComponents || {};

console.log('🔄 Greeting card component loader started');

// List of all available greeting card components
const componentFiles = [
    'layout', 'hero', 'greeting', 'wish', 'note', 'photo', 'quote', 'poem',
    'memories', 'gallery', 'video', 'music', 'countdown', 'signature', 'footer'
];

// Function to check if all components are loaded
function checkComponentsLoaded() {
    console.log('🔍 Checking if greeting card components are loaded...');
    ...
    console.log(`Greeting card components loaded: ${loaded.length}/${componentFiles.length}`);

    if (missing.length > 0) {
        console.warn('Missing greeting card components:', missing.join(', '));
    } else {
        console.log('✅ All greeting card components loaded successfully!');
    }
